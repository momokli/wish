...
reset_state() {
    ...
    if ! ssh -o ConnectTimeout=5 "$MUSIC_HOST" "sudo systemctl stop wish.service && rm -f /home/momo/wish/wish.db* /opt/music-stack/wish-downloads/*.mp3 /opt/music-stack/deemix-wish-config/queue/*.json && docker stop deemix-wish && rm -rf /opt/music-stack/deemix-wish-config/queue && mkdir /opt/music-stack/deemix-wish-config/queue && docker start deemix-wish && sleep 3 && sudo systemctl start wish.service && echo ok" 2>/dev/null; then
